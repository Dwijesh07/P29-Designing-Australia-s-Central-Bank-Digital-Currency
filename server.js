...
app.get('/api/transactions/export/pdf', authenticate, async (req, res) => {
  try {
    if (req.user.role !== 'austrac_admin') {
      return res.status(403).json({
        success: false,
        error: 'Only AUSTRAC admin can export transaction reports'
      });
    }

    const { suspiciousOnly, bank, minRisk } = req.query;
    let exportData = [...transactionHistory];

    if (suspiciousOnly === 'true') {
      exportData = exportData.filter(tx => tx.suspicious);
    }

    if (bank && bank !== 'All') {
      exportData = exportData.filter(tx => tx.fromBank === bank || tx.toBank === bank);
    }

    if (minRisk) {
      const riskValue = Number(minRisk);
      if (!Number.isNaN(riskValue)) {
        exportData = exportData.filter(tx => Number(tx.riskScore || 0) >= riskValue);
      }
    }
    ...
    const filename = `eaud-transaction-report-${Date.now()}.pdf`;
    ...
    doc.fontSize(20).text('eAUD Transaction Monitoring Report', {
      align: 'center'
    });
    ...
    doc.text('Prepared for: AUSTRAC Admin', { align: 'center' });
    doc.moveDown(1);
    ...
    const suspiciousCount = exportData.filter(tx => tx.suspicious).length;

    const avgRisk = exportData.length
      ? Math.round(
          exportData.reduce((sum, tx) => sum + Number(tx.riskScore || 0), 0) /
            exportData.length
        )
      : 0;

    doc.fontSize(12).text(`Total Transactions: ${exportData.length}`);
    doc.text(`Total Value: ${totalAmount.toLocaleString()} eAUD`);
    doc.text(`Suspicious Transactions: ${suspiciousCount}`);
    doc.text(`Average Risk Score: ${avgRisk}%`);
    doc.moveDown(1);

    const columns = [
      { title: 'Time', x: 40, width: 115 },
      { title: 'From', x: 155, width: 95 },
      { title: 'To', x: 250, width: 95 },
      { title: 'Amount', x: 345, width: 90 },
      { title: 'Status', x: 435, width: 80 },
      { title: 'Risk', x: 515, width: 55 },
      { title: 'Flags', x: 570, width: 235 }
    ];
    ...
    function drawHeader() {
      ...
      columns.forEach(col => {
        doc.text(col.title, col.x, y, { width: col.width });
      });
      y += 18;
      doc.moveTo(40, y - 5).lineTo(800, y - 5).stroke();
      doc.font('Helvetica');
    }
    ...
    exportData.forEach(tx => {
      ...
      const flags =
        Array.isArray(tx.flags) && tx.flags.length
          ? tx.flags.join(', ')
          : '-';
      ...
      const row = [
        ...
        tx.from || '-',
        tx.to || '-',
        `${Number(tx.amount || 0).toLocaleString()} eAUD`,
        tx.status || '-',
        `${tx.riskScore || 0}%`,
        flags
      ];
    ...
    if (exportData.length === 0) {
      doc.fontSize(10).text(
        'No transactions matched the selected export filters.',
        40,
        y
      );
    }
    ...
    doc.fontSize(8).text(
      'Confidential regulatory monitoring report generated by the eAUD CBDC prototype.',
      40,
      560,
      { align: 'center' }
    );
