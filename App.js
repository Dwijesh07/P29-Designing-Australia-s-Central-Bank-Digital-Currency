...
function App() {
  ...
  try {
    ...
    if (showSuspicious) {
      params.append('suspiciousOnly', 'true');
    }

    const response = await axios.get(
      `${API_URL}/transactions/export/pdf?${params.toString()}`,
      {
        responseType: 'blob'
      }
    );
    ...
    link.download = `eaud-transaction-report-${new Date()
      .toISOString()
      .slice(0, 10)}.pdf`;
    ...
    setMessage('PDF transaction report exported successfully.');
    setMessageType('success');
  } catch (error) {
    console.error('PDF export error:', error);
    setMessage(error.response?.data?.error || 'PDF export failed.');
      ...
      <div className="transactions-section">
        <div className="card full-width">
          <div className="transactions-header">
            <h2>
              <FaChartLine />
              {isAUSTRAC ? ' AML Transaction Monitoring' : ' Recent Transactions'}
            </h2>

            {isAUSTRAC && (
              <button className="btn-export-pdf" onClick={exportTransactionsPdf}>
